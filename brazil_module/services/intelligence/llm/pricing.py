...
PRICING: dict[str, dict[str, Price]] = {
    ...
    "google": {
        # Flash 3.x: promotional price, doubles on 2027-01-01.
        ...
        "gemini-2.5-pro": Price(1.25, 10.0, 0.125),
        "gemini-3.1-pro-preview": Price(2.0, 12.0, 0.20),
...
def price_for(provider: str, model: str) -> Price:
    ...
    if table is None:
        _report(f"Unknown LLM provider '{provider}' — priced as the dearest model known")
    ...
    _report(f"Unknown model '{model}' for provider '{provider}' — priced as that provider's dearest")
...
def calculate_cost_usd(
    ...
    """What one call cost. `tokens_in` excludes `cached_tokens`."""
...
def _report(message: str) -> None:
    """A price we had to guess is worth knowing about; it is not worth failing
    a call over, and it is not worth losing a cost record either."""
    try:
