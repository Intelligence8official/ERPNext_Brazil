                                ...
                                if (typeof r.message === 'object') {
                                    for (let key in r.message) {
                                        ...
                                        if (result.status === 'rate_limited') {
                                            ...
                                            msg += `&nbsp;&nbsp;<span style="color:red">❌ Error: ${result.message || 'Unknown error'}</span><br>`;
                                        } else {
                                            msg += `&nbsp;&nbsp;Fetched: ${result.fetched || 0}`;
                                            msg += `, Created: ${result.created || 0}`;
                                            if (result.skipped) {
                                                msg += `, Skipped (duplicates): ${result.skipped}`;
                                            }
                                            if (result.events) {
