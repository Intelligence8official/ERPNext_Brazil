...
class TestCheckEmails(unittest.TestCase):
    ...
    def test_processes_communications(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.process_email") as mock_pe:
            check_emails()
...
class TestProcessEmail(unittest.TestCase):
    ...
    def test_routes_xml_attachment(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.process_xml_attachment", return_value=True) as mock_xml:
    ...
    def test_routes_pdf_attachment(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.process_pdf_attachment", return_value=1) as mock_pdf:
    ...
    def test_routes_zip_attachment(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.process_zip_attachment", return_value=3) as mock_zip:
    ...
    def test_error_logged_and_processing_continues(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.process_xml_attachment", side_effect=side_effect_fn):
...
class TestProcessXmlAttachment(unittest.TestCase):
    ...
    def test_valid_xml_creates_nf(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=xml_bytes):
            with patch("brazil_module.services.fiscal.email_monitor.create_nf_from_xml", return_value=True) as mock_create:
    ...
    def test_empty_file_returns_false(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=None):
            result = process_xml_attachment(attachment, comm, settings)
    ...
    def test_latin1_fallback(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=latin1_content):
            with patch("brazil_module.services.fiscal.email_monitor.create_nf_from_xml", return_value=True) as mock_create:
                result = process_xml_attachment(attachment, comm, settings)
                self.assertTrue(result)
...
class TestProcessPdfAttachment(unittest.TestCase):
    ...
    def test_embedded_xml_processed(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=b"fake pdf"):
            with patch("brazil_module.services.fiscal.email_monitor.extract_xml_from_pdf", return_value=["<nfeProc>...</nfeProc>"]):
                with patch("brazil_module.services.fiscal.email_monitor.create_nf_from_xml", return_value=True) as mock_create:
    ...
    def test_chave_from_text_extraction(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=b"fake pdf"):
            with patch("brazil_module.services.fiscal.email_monitor.extract_xml_from_pdf", return_value=[]):
                with patch("brazil_module.services.fiscal.email_monitor.extract_data_from_pdf", return_value=pdf_data):
                    with patch("brazil_module.services.fiscal.email_monitor.create_nf_from_pdf_data", return_value=True) as mock_create:
                        result = process_pdf_attachment(attachment, comm, settings)
                        self.assertEqual(result, 1)
                        mock_create.assert_called_once()
    ...
    def test_international_invoice(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=b"fake pdf"):
            with patch("brazil_module.services.fiscal.email_monitor.extract_xml_from_pdf", return_value=[]):
                with patch("brazil_module.services.fiscal.email_monitor.extract_data_from_pdf", return_value=None):
                    with patch("brazil_module.services.fiscal.email_monitor.extract_international_invoice", return_value=invoice_data):
                        with patch("brazil_module.services.fiscal.email_monitor.create_nf_from_invoice_data", return_value=True) as mock_create:
                            result = process_pdf_attachment(attachment, comm, settings)
                            self.assertEqual(result, 1)
                            mock_create.assert_called_once()
    ...
    def test_no_match_returns_zero(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=b"fake pdf"):
            with patch("brazil_module.services.fiscal.email_monitor.extract_xml_from_pdf", return_value=[]):
                with patch("brazil_module.services.fiscal.email_monitor.extract_data_from_pdf", return_value=None):
                    with patch("brazil_module.services.fiscal.email_monitor.extract_international_invoice", return_value=None):
                        result = process_pdf_attachment(attachment, comm, settings)
...
class TestProcessZipAttachment(unittest.TestCase):
    ...
    def test_zip_with_xmls(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=zip_bytes):
            with patch("brazil_module.services.fiscal.email_monitor.create_nf_from_xml", return_value=True) as mock_create:
    ...
    def test_zip_with_pdfs(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=zip_bytes):
            with patch("brazil_module.services.fiscal.email_monitor.extract_xml_from_pdf", return_value=["<nfeProc>data</nfeProc>"]):
                with patch("brazil_module.services.fiscal.email_monitor.create_nf_from_xml", return_value=True) as mock_create:
                    result = process_zip_attachment(attachment, comm, settings)
                    self.assertEqual(result, 1)
    ...
    def test_bad_zip_returns_zero(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.get_file_content", return_value=b"not a zip file"):
            result = process_zip_attachment(attachment, comm, settings)
            self.assertEqual(result, 0)
            frappe.log_error.assert_called()
...
class TestCreateNfFromXml(unittest.TestCase):
    ...
    def test_creates_new_nf(self):
        ...
        with patch("brazil_module.services.fiscal.xml_parser.NFXMLParser", return_value=mock_parser):
...
class TestExtractDataFromPdf(unittest.TestCase):
    ...
    def test_extracts_chave(self):
        ...
        with patch("brazil_module.services.fiscal.email_monitor.PdfReader", mock_reader.__class__, create=True):
            # Patch at module level
            import brazil_module.services.fiscal.email_monitor as em_mod
            ...
            with patch.dict("sys.modules", {"pypdf": MagicMock()}):
                with patch("brazil_module.services.fiscal.email_monitor.PdfReader", mock_pdf_class, create=True):
...
class TestGetFileContent(unittest.TestCase):
    ...
    @patch("brazil_module.services.fiscal.email_monitor.os.path.exists", return_value=True)
    ...
    @patch("brazil_module.services.fiscal.email_monitor.os.path.exists", return_value=False)
