...
from Brazil_Module.fiscal.doctype.nota_fiscal.nota_fiscal import NotaFiscal
from Brazil_Module.fiscal.doctype.nf_company_settings.nf_company_settings import NFCompanySettings
from Brazil_Module.fiscal.doctype.nota_fiscal_settings.nota_fiscal_settings import NotaFiscalSettings
from Brazil_Module.fiscal.doctype.nf_import_log.nf_import_log import NFImportLog
from Brazil_Module.fiscal.doctype.nota_fiscal_evento.nota_fiscal_evento import NotaFiscalEvento
from Brazil_Module.fiscal.doctype.nota_fiscal_item.nota_fiscal_item import NotaFiscalItem
...
class TestNotaFiscalValidation(unittest.TestCase):
    ...
    def test_valid_chave_passes(self):
        ...
        with patch("Brazil_Module.utils.chave_acesso.validate_chave_acesso", return_value=True):
            nf.validate()
    ...
    def test_invalid_chave_throws(self):
        ...
        with patch("Brazil_Module.utils.chave_acesso.validate_chave_acesso", return_value=False):
            with patch("Brazil_Module.utils.chave_acesso.clean_chave", return_value="12345"):
    ...
    def test_cnpj_warning_on_invalid(self):
        ...
        with patch("Brazil_Module.utils.chave_acesso.validate_chave_acesso", return_value=True):
            with patch("Brazil_Module.utils.cnpj.validate_cnpj", return_value=False):
                with patch("Brazil_Module.utils.cnpj.clean_cnpj", return_value="12345678000100"):
                    nf.validate()
    ...
    def test_before_insert_sets_defaults(self):
        ...
        nf = self._make_nf()
        with patch("Brazil_Module.fiscal.doctype.nota_fiscal.nota_fiscal.now_datetime", return_value=_NOW):
            nf.before_insert()
...
class TestNFCompanySettingsValidation(unittest.TestCase):
    ...
    def test_cnpj_must_be_14_digits(self):
        ...
        with patch("Brazil_Module.utils.cnpj.clean_cnpj", return_value="1234"):
            with self.assertRaises(Exception):
                s.validate()
        frappe.throw.assert_called_once()
    ...
    def test_valid_cnpj_stored_cleaned(self):
        ...
        with patch("Brazil_Module.utils.cnpj.clean_cnpj", return_value="12345678000195"):
            with patch("Brazil_Module.utils.cnpj.validate_cnpj", return_value=True):
                s.validate()
    ...
    def test_cert_validation_success(self):
        ...
        with patch("Brazil_Module.services.fiscal.cert_utils.validate_pfx_certificate", return_value="2025-12-31"):
            s.validate()
...
class TestNFImportLogMethods(unittest.TestCase):
    ...
    def test_before_insert_defaults_started_at(self):
        ...
        log = self._make_log()
        with patch("Brazil_Module.fiscal.doctype.nf_import_log.nf_import_log.now_datetime", return_value=_NOW):
            log.before_insert()
...
class TestNotaFiscalEventoValidation(unittest.TestCase):
    ...
    def test_before_insert_defaults_event_date(self):
        ...
        e = self._make_evento()
        with patch("Brazil_Module.fiscal.doctype.nota_fiscal_evento.nota_fiscal_evento.now_datetime", return_value=_NOW):
            e.before_insert()
